...
// Expo push token helpers (no FCM, Expo only)
...
// Register Expo push token with backend (Expo only, strict error handling)
export const registerExpoPushToken = async (expoPushToken) => {
  if (!expoPushToken || typeof expoPushToken !== 'string') {
    throw new Error('Invalid Expo push token');
  }
  const res = await request("/api/notifications/register-token", {
    method: "POST",
...
// Unregister Expo push token (Expo only)
export const unregisterExpoPushToken = async (expoPushToken) => {
  ...
  if (token) await clearExpoPushToken();
  return res;
};
...
export const logout = async () => {
  ...
  try {
    ...
    if (expoPushToken) {
      await unregisterExpoPushToken(expoPushToken);
    } else {
      // If token wasn't stored locally, still try to clear on server.
