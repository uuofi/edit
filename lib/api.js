...
function getConfiguredApiBaseUrl() {
  ...
  // ملاحظة: نتجنّب optional chaining هنا (process.env?.X) لأن بعض إصدارات
  // babel-preset-expo تحقن EXPO_PUBLIC_* فقط مع الوصول المباشر (process.env.X)،
  // فمع ?. قد تسقط القيمة وتُستخدم القيمة الافتراضية بدل رابط التشغيل.
  const envUrl =
...
const ONBOARDING_SEEN_KEY = "medicare_onboarding_seen";
...
export const clearOnboardingSeen = () =>
...
// 🖥️ شاشة عرض أسماء المرضى — يُنشئ/يجلب رمز الشاشة ويرجع رابطها الجاهز للفتح.
export const generatePatientDisplay = () =>
  request("/api/display/generate", { method: "POST" });
...
export const setBlock = (patientId, { blockBooking, blockChat } = {}) => {
  const payload = { patientId };
  if (typeof blockBooking === "boolean") payload.blockBooking = blockBooking;
  if (typeof blockChat === "boolean") payload.blockChat = blockChat;
...
export const fetchBlockedPatients = ({ scope = "all", search = "" } = {}) => {
  const parts = [];
  const normalizedScope = String(scope || "").trim();
  const normalizedSearch = String(search || "").trim();

  if (normalizedScope) {
    parts.push(`scope=${encodeURIComponent(normalizedScope)}`);
  }
  if (normalizedSearch) {
...
const buildChatQuery = ({ appointmentId, patientId, limit, before } = {}) => {
  const parts = [];
  ...
  if (Number.isFinite(Number(limit))) {
    parts.push(`limit=${encodeURIComponent(String(Math.max(1, Math.min(Number(limit), 100))))}`);
  }
  if (before) {
...
export const fetchChatContext = (params = {}) =>
  request(`/api/chat/context${buildChatQuery(params)}`);

export const fetchChatMessages = (params = {}) =>
  request(`/api/chat/messages${buildChatQuery(params)}`);

export const fetchDoctorChatConversations = ({ limit } = {}) => {
  const parsedLimit = Number(limit);
  const normalizedLimit = Number.isFinite(parsedLimit)
    ? Math.max(1, Math.min(Math.floor(parsedLimit), 100))
    : null;

  const query = normalizedLimit ? `?limit=${encodeURIComponent(String(normalizedLimit))}` : "";
  return request(`/api/chat/conversations${query}`);
};

export const setChatPublicKey = (publicKey) =>
  request("/api/chat/public-key", {
    method: "POST",
    body: JSON.stringify({ publicKey: String(publicKey || "").trim() }),
  });

export const getChatPublicKey = () => request("/api/chat/public-key");
...
export const uploadChatImage = async ({ uri, appointmentId, patientId } = {}) => {
  const imageUri = String(uri || "").trim();
  if (!imageUri) {
    throw new ApiError("Missing image uri", 400, {});
  }

  const headers = await buildAuthHeaders();
  const cleanedUri = imageUri.split("?")[0].split("#")[0];
  const ext = cleanedUri.split(".").pop()?.toLowerCase() || "jpg";
  const safeExt = /^(jpg|jpeg|png|webp|gif)$/.test(ext) ? ext : "jpg";
  const fileName = `chat-${Date.now()}.${safeExt}`;
  ...
  if (normalizedAppointmentId) {
    form.append("appointmentId", normalizedAppointmentId);
  }
  if (normalizedPatientId) {
    form.append("patientId", normalizedPatientId);
  }
...
export const markMessagesRead = (payload) =>
  request("/api/chat/messages/read", {
    method: "POST",
    body: JSON.stringify(payload || {}),
  });

export const markMessagesDelivered = (payload) =>
  request("/api/chat/messages/delivered", {
    method: "POST",
    body: JSON.stringify(payload || {}),
  });

export const sendTypingIndicator = (payload) =>
  request("/api/chat/typing", {
    method: "POST",
    body: JSON.stringify(payload || {}),
  });

export const getChatSettings = () =>
  request("/api/chat/settings");
